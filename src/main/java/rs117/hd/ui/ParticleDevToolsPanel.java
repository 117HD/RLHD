...
public class ParticleDevToolsPanel extends PluginPanel {
	...
	public ParticleDevToolsPanel(HdPlugin plugin, ParticleGizmoOverlay particleGizmoOverlay) {
		...
		JSpinner directionYaw = new JSpinner(new SpinnerNumberModel(1248, 0, ANGLE_UNITS_PER_CIRCLE, 1));
		setSingleLineHeight(directionYaw);
		generalSection.add(directionYaw);
		generalSection.add(new JLabel("Direction Pitch:"));
		JSpinner directionPitch = new JSpinner(new SpinnerNumberModel(30, 0, ANGLE_UNITS_PER_CIRCLE, 1));
	...
	private void showEditorFor(ParticleEmitterDefinition def) {
		...
		heightSpinner.setValue(def.heightOffset);
		directionYawSpinner.setValue(def.directionYaw);
		directionPitchSpinner.setValue(def.directionPitch);
		spreadYawMinSpinner.setValue(def.spreadYawMin);
		spreadYawMaxSpinner.setValue(def.spreadYawMax);
		spreadPitchMinSpinner.setValue(def.spreadPitchMin);
		spreadPitchMaxSpinner.setValue(def.spreadPitchMax);
		minSpeedSpinner.setValue(def.minSpeed);
		maxSpeedSpinner.setValue(def.maxSpeed);
		targetSpeedSpinner.setValue(def.targetSpeed);
		speedTransitionPctSpinner.setValue(def.speedTransitionPercent);
		minScaleSpinner.setValue(def.minScale);
		...
		colorTransitionSpinner.setValue(def.colourTransitionPercent);
		alphaTransitionSpinner.setValue(def.alphaTransitionPercent);
		uniformColorVariationCheckBox.setSelected(def.uniformColourVariation);
		minDelaySpinner.setValue(def.minEmissionDelay);
		maxDelaySpinner.setValue(def.maxEmissionDelay);
		minSpawnSpinner.setValue(def.minSpawnCount);
		maxSpawnSpinner.setValue(def.maxSpawnCount);
		initialSpawnSpinner.setValue(def.initialSpawnCount);
		textureCombo.setSelectedItem(def.texture != null && !def.texture.isEmpty() ? def.texture : "");
		} finally {
			loadingEditor = false;
		}
	...
	private void chooseTargetColour() {
		...
		if (chosen == null) return;
		def.targetColourArgb = fromColorToArgb(chosen);
	...
	private void onExportJsonClicked() {
		...
		map.put("heightOffset", def.heightOffset);
		map.put("directionYaw", def.directionYaw);
		map.put("directionPitch", def.directionPitch);
		map.put("spreadYawMin", def.spreadYawMin);
		map.put("spreadYawMax", def.spreadYawMax);
		map.put("spreadPitchMin", def.spreadPitchMin);
		map.put("spreadPitchMax", def.spreadPitchMax);
		map.put("minSpeed", def.minSpeed);
		map.put("maxSpeed", def.maxSpeed);
		map.put("targetSpeed", def.targetSpeed);
		map.put("speedTransitionPercent", def.speedTransitionPercent);
		map.put("minScale", def.minScale);
		...
		map.put("minEmissionDelay", def.minEmissionDelay);
		map.put("maxEmissionDelay", def.maxEmissionDelay);
		map.put("minSpawnCount", def.minSpawnCount);
		map.put("maxSpawnCount", def.maxSpawnCount);
		map.put("initialSpawnCount", def.initialSpawnCount);
	...
	private void applyAllToDefinition() {
		...
		def.heightOffset = ((Number) heightSpinner.getValue()).intValue();
		def.directionYaw = ((Number) directionYawSpinner.getValue()).intValue();
		def.directionPitch = ((Number) directionPitchSpinner.getValue()).intValue();
		def.spreadYawMin = ((Number) spreadYawMinSpinner.getValue()).intValue();
		def.spreadYawMax = ((Number) spreadYawMaxSpinner.getValue()).intValue();
		def.spreadPitchMin = ((Number) spreadPitchMinSpinner.getValue()).intValue();
		def.spreadPitchMax = ((Number) spreadPitchMaxSpinner.getValue()).intValue();
		def.minSpeed = ((Number) minSpeedSpinner.getValue()).intValue();
		def.maxSpeed = ((Number) maxSpeedSpinner.getValue()).intValue();
		def.targetSpeed = ((Number) targetSpeedSpinner.getValue()).intValue();
		def.speedTransitionPercent = ((Number) speedTransitionPctSpinner.getValue()).intValue();
		def.minScale = ((Number) minScaleSpinner.getValue()).intValue();
		def.maxScale = ((Number) maxScaleSpinner.getValue()).intValue();
		def.targetScale = ((Number) targetScaleSpinner.getValue()).intValue();
		def.scaleTransitionPercent = ((Number) scaleTransitionSpinner.getValue()).intValue();
		...
		def.minColourArgb = fromColorToArgb(minC);
		def.maxColourArgb = fromColorToArgb(maxC);
		Color tgt = targetColourButton.getBackground() instanceof Color ? (Color) targetColourButton.getBackground() : null;
		def.targetColourArgb = (tgt != null && tgt != Color.GRAY) ? fromColorToArgb(tgt) : 0;
		def.colourTransitionPercent = ((Number) colorTransitionSpinner.getValue()).intValue();
		def.alphaTransitionPercent = ((Number) alphaTransitionSpinner.getValue()).intValue();
		def.uniformColourVariation = uniformColorVariationCheckBox.isSelected();
		def.minEmissionDelay = ((Number) minDelaySpinner.getValue()).intValue();
		def.maxEmissionDelay = ((Number) maxDelaySpinner.getValue()).intValue();
		def.minSpawnCount = ((Number) minSpawnSpinner.getValue()).intValue();
		def.maxSpawnCount = ((Number) maxSpawnSpinner.getValue()).intValue();

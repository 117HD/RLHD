...
public class Zone implements Destructible {
	...
	public void initialize(WorldViewContext viewCtx, SceneContext sceneCtx, int x, int z, GpuIntBuffer opaqueStaging, GpuIntBuffer alphaStaging, GpuIntBuffer tboStaging) {
		...
		if (sizeIntsOpaque > 0) {
			vboO = new GLBuffer("Zone::VBO::Opaque", GL_ARRAY_BUFFER, GL_STATIC_DRAW);
			vboO.initialize(opaqueStaging.getBuffer());
			glVao = glGenVertexArrays();
			setupVao(glVao, vboO.id, vboM.id);
		}

		if (sizeIntsAlpha > 0) {
			vboA = new GLBuffer("Zone::VBO::Alpha", GL_ARRAY_BUFFER, GL_STATIC_DRAW);
			vboA.initialize(alphaStaging.getBuffer());
			glVaoA = glGenVertexArrays();
			setupVao(glVaoA, vboA.id, vboM.id);
		}

		if (sizeIntsFace > 0) {
			tboF = new GLTextureBuffer("Zone::TBO", GL_STATIC_DRAW);
			tboF.initialize(tboStaging.getBuffer());
		}

		if(sizeIntsAlpha > 0 && sizeIntsFace > 0) {
			for (int i = 0; i < alphaModels.size(); i++) {
				final Zone.AlphaModel m = alphaModels.get(i);
				m.vao = glVaoA;
				m.tboF = tboF.getTexId();
			}
		}

		setMetadata(viewCtx, sceneCtx, x, z);
		initialized = true;

		viewCtx.bufferInit += System.nanoTime() - start;
	}
	...
	public void destroy() {
		...
		if (uploadJob != null) {
			uploadJob.cancel();
			DestructibleHandler.destroy(uploadJob.getZone());
			uploadJob = null;
		}

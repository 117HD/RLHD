...
public class WorldViewContext {
	...
	void handleZoneSwap(int zx, int zz, boolean queue) {
		...
		if (!uploadTask.isQueued()) {
			if (queue && uploadTask.getRevealAfterTimestampMs() < System.currentTimeMillis()) {
				log.trace("queueing zone({}): [{}-{},{}]", uploadTask.getZone().hashCode(), worldViewId, zx, zz);
		...
		if (uploadTask.isDone()) {
			...
			if (uploadTask.ranToCompletion() && !uploadTask.wasCancelled()) {
				log.trace("swapping zone({}): [{}-{},{}]", uploadTask.getZone().hashCode(), worldViewId, zx, zz);

				Zone prevZone = curZone;
				// Swap the zone out with the one we just uploaded
				zones[zx][zz] = curZone = uploadTask.getZone();
				if (prevZone != curZone) {
	...
	void invalidateZone(int zx, int zz) {
		...
		if (curZone.uploadJob != null) {
			Zone pendingZone = curZone.uploadJob.getZone();
			log.trace(
			...
			revealAfterTimestampMs = curZone.uploadJob.getRevealAfterTimestampMs();
		...
		curZone.uploadJob = ZoneUploadJob.build(this, sceneContext, newZone, zx, zz, revealAfterTimestampMs);

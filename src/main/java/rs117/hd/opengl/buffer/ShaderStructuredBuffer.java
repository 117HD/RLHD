...
public class ShaderStructuredBuffer extends StructuredBuffer<GLBuffer> {
	// Force using SSBO path when debugging. If true, texture buffer path disabled.
	private static final boolean FORCE_TEXTURE_BUFFER = false;

	// We use RGB texels for broad macOS compatibility (GL_RGB32I), which pack 3 ints per texel.
	private static final int TEXEL_INT_COMPONENTS = 3; // number of 32-bit ints per texel for RGB32I

	private final boolean isTextureBuffer;
	private int textureBufferId;
	private int currentCapacity;

	private static boolean supportsShaderStorage() {

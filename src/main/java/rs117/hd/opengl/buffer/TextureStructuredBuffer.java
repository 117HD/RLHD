...
public class TextureStructuredBuffer extends StructuredBuffer<GLBuffer> {
	...
	public String generateGetter(String structName, String functionName) {
		...
		sb.append(structName).append(" ").append(functionName).append("(int index, isampler1D sampler) {\n");
		...
		for (Property prop : def.properties) {
			...
			if (texelCount == 1) {
				sb.append(glslType.equals("float") ?
						"intBitsToFloat(texelFetch(" : "texelFetch(sampler, index + ").append(texelIndex).append(", 0).r");
				...
				for (int i = 0; i < texelCount; i++) {
					if (i > 0) sb.append(", ");
					sb.append("texelFetch(sampler, index + ").append(texelIndex + i).append(", 0).r");
				}
				sb.append(")");
			}
			sb.append(";\n");
